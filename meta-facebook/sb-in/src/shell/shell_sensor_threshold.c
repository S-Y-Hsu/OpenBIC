...
void cmd_set_sensor_threshold(const struct shell *shell, size_t argc, char **argv)
{
	...
	int sensor_id = strtol(argv[1], NULL, 16);

	if (!sensor_id || sensor_id > sensor_max_count) {
		shell_error(shell, "Help: Sensor ID: 0x%x is higher than 0x%x or lower than 0x1",
			    sensor_id, sensor_max_count);
		return;
	}

	int result = 0;
	result = check_supported_threshold_with_sensor_id(sensor_id);
	if (result != 0) {
		shell_error(shell, "Sensor ID 0x%x unsupported thresholds", sensor_id);
		return;
	}
	...
	if (strcmp(threshold_type, "UCT") == 0) {
		...
		result = change_pdr_table_critical_low_with_sensor_id(sensor_id, threshold_value);
		if (result != 0) {
	...
	shell_print(shell, "sensor_threshold set 0x%x %s %d success!", sensor_id, threshold_type,
		    value);
...
void cmd_get_sensor_threshold(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (strcmp(threshold_all, "All") == 0 || strcmp(threshold_all, "all") == 0) {
		for (int i = SENSOR_NUM_TOP_INLET_TEMP_C; i <= sensor_max_count; i++) {
		...
		result = get_pdr_table_critical_high_and_low_with_sensor_id(
			sensor_id, &critical_high, &critical_low);
		if (result != 0) {
		...
		shell_print(
			shell,
			"sensor ID: 0x%x  |  sensor name: %-50s  |  critical high: %10.3f  |  critical low: %10.3f",
			sensor_id, sensor_name, critical_high, critical_low);
	}
